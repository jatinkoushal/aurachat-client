...
export default function People() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError('Could not load users — server may be starting up. Please wait a moment then retry.');
    } finally { setLoading(false); }
  }, []);

  useEffect(() => { load(); }, [load]);

  // Search applies to whichever list is active
  ...
  // Apply search to each list
  const filt = arr => q ? arr.filter(x => (x.username||'').toLowerCase().includes(q)) : arr;
  const filtAll = filt(allUsers);

  // ── Actions — all update state instantly, then reload for consistency ────────
  ...
  const muteUser = async (relId, userId) => {
    ...
    setAllUsers(prev => prev.map(u => u.id===userId ? {...u, rel_status:'muted'} : u));
    try { await api.patch(`/api/relationships/${relId}/mute`); }
    catch(err) { alert(err.response?.data?.error||'Failed'); load(); }
  };

  const unmuteUser = async (relId, userId) => {
    setRelationships(prev => prev.map(r => r.id===relId ? {...r, status:'accepted'} : r));
  ...
  return (
    <div style={s.page}>
      ...
      <div style={s.tabs}>
        {tabs.map(t => (
          <button key={t.key} onClick={() => setTab(t.key)}
            style={{ ...s.tabBtn, ...(tab===t.key ? s.tabOn : {}), ...(t.highlight ? { color:'var(--warning)' } : {}) }}>
            {t.label}
      ...
      <div style={s.list}>
        {loading && (
          <div style={s.center}>
            <div style={{ width:28,height:28,border:'3px solid var(--border)',borderTopColor:'var(--accent-primary)',borderRadius:'50%',animation:'spin 0.8s linear infinite' }} />
        ...
        {!loading && !error && tab==='all' && (
          filtAll.length === 0
            ...
            : filtAll.map(u => (
              <Row key={u.id} user={u} online={isOnline(u.id)}>
                {!u.rel_status &&
        ...
        {!loading && !error && tab==='friends' && (
          filt(friends).length===0
            ? <div style={s.empty}>{q ? `No friends matching "${search}"` : 'No friends yet — go to Everyone tab!'}</div>
            : filt(friends).map(r => (
              <Row key={r.id} user={r} online={isOnline(r.user_id)}>
        ...
        {!loading && !error && tab==='requests' && (
          filt(incoming).length===0
            ? <div style={s.empty}>{q ? `No results for "${search}"` : 'No incoming requests'}</div>
            : filt(incoming).map(r => (
              <Row key={r.id} user={r} online={isOnline(r.user_id)} badge="Wants to be friends">
        ...
        {!loading && !error && tab==='sent' && (
          filt(sent).length===0
            ? <div style={s.empty}>{q ? `No results for "${search}"` : 'No sent requests'}</div>
            : filt(sent).map(r => (
              <Row key={r.id} user={r} online={isOnline(r.user_id)} badge="Request pending…">
        ...
        {!loading && !error && tab==='muted' && (
          filt(muted).length===0
            ? <div style={s.empty}>{q ? `No results for "${search}"` : 'No muted users'}</div>
            : filt(muted).map(r => (
              <Row key={r.id} user={r} online={isOnline(r.user_id)} badge="🔇 Muted">
        ...
        {!loading && !error && tab==='blocked' && (
          filt(blocked).length===0
            ? <div style={s.empty}>{q ? `No results for "${search}"` : 'No blocked users'}</div>
            : filt(blocked).map(r => (
              // FIX: use r (the other person in the relationship), not current user
              <Row key={r.id} user={r} online={false} badge="🚫 Blocked">
...
const s = {
  ...
  bio:      { fontSize:11, color:'var(--text-muted)', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', maxWidth:'100%' },

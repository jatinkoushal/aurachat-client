...
export default function Groups() {
  ...
  const load = async () => {
    try {
      ...
      setLoadErr('');
    } catch (e) { setLoadErr('Failed to load groups'); }
  };
  useEffect(() => { load(); }, []);
    ...
    <div style={s.page}>
      <div style={s.header}>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <button className="btn-icon" title="Join by code" onClick={() => setView(v => v === 'join' ? 'list' : 'join')}>🔗</button>
          {invites.length > 0 && (
      ...
      {view === 'join' && (
        <div style={s.panel}>
          ...
          <form onSubmit={joinGroup} style={{ display: 'flex', gap: 8 }}>
            <input className="input" placeholder="AB12CD34" value={joinCode}
          ...
          {invites.map(inv => (
            <div key={inv.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 0', borderBottom: '1px solid var(--border)' }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div className="text-muted">From {inv.inviter_username}</div>
              </div>
        ...
        <div style={{ ...s.panel, flex: 1, overflowY: 'auto' }}>
          <form onSubmit={createGroup} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div>
              ...
              <input className="input" placeholder="Min 2 characters" value={form.name}
            ...
            {friends.length > 0 ? (
              ...
              <div style={{ fontSize: 13, color: 'var(--text-muted)', padding: '8px 12px', background: 'var(--bg-card)', borderRadius: 'var(--radius-sm)' }}>
                💡 Add friends first to include them. You can invite them later too.
              </div>
            )}
            <button className="btn btn-primary" type="submit"
              disabled={creating || !form.name.trim() || form.name.trim().length < 2} style={{ padding: 12, fontSize: 15 }}>
      ...
      {view === 'list' && (
        <div style={{ flex: 1, overflowY: 'auto', padding: 8 }}>
          {loadErr && <div style={{ color: 'var(--danger)', padding: 12, fontSize: 13 }}>{loadErr} <button className="btn btn-ghost" onClick={load} style={{ marginLeft: 8, padding: '4px 10px', fontSize: 12 }}>Retry</button></div>}
          {groups.length === 0 && !loadErr && (
            <div style={s.empty}>
              <div style={{ fontSize: 48, marginBottom: 12 }}>🏠</div>
              <div style={{ fontWeight: 600, marginBottom: 4 }}>No groups yet</div>
              <div style={{ fontSize: 13, color: 'var(--text-muted)' }}>Create or join a group to get started</div>
            </div>
          )}
          {groups.map(g => (
            <div key={g.id} onClick={() => setSelected(g)}
              style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 8px', borderRadius: 'var(--radius-sm)', cursor: 'pointer', borderBottom: '1px solid var(--border)' }}
              ...
              {g.admin_id === user?.id && (
                <span style={{ fontSize: 10, fontWeight: 700, background: 'var(--accent-primary)', color: '#fff', padding: '2px 8px', borderRadius: 999, flexShrink: 0 }}>Admin</span>
              )}
            </div>
          ))}
        </div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', width: '100%', height: '100%' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 16px', background: 'var(--bg-secondary)', borderBottom: '1px solid var(--border)', flexShrink: 0 }}>
        ...
        <div style={{ flex: 1, cursor: 'pointer' }} onClick={() => { setShowInfo(p => !p); setEditMode(false); }}>
          ...
          <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>
            {typing ? '✏️ typing...' : `${details?.members?.length || 0} members • tap for info`}
          </div>
        </div>
        <button className="btn-icon" title="Leave group" onClick={leaveGroup} style={{ color: 'var(--danger)' }}>🚪</button>
        {details?.group?.invite_code && (
          <button className="btn-icon" title="Copy invite code"
            onClick={() => { navigator.clipboard.writeText(details.group.invite_code); alert(`Code: ${details.group.invite_code}`); }}>🔗</button>
        )}
      </div>
      ...
      {showInfo && details && (
        <div style={{ background: 'var(--bg-secondary)', borderBottom: '1px solid var(--border)', padding: 14, maxHeight: 320, overflowY: 'auto', flexShrink: 0 }}>
          ...
          {/* Admin: edit group */}
          {isAdmin && !editMode && (
            <button className="btn btn-ghost" onClick={() => setEditMode(true)} style={{ marginBottom: 12, padding: '6px 14px', fontSize: 12, width: '100%' }}>
              ✏️ Edit Group Name / Photo
            </button>
          )}

          {isAdmin && editMode && (
            <div style={{ marginBottom: 14, padding: 12, background: 'var(--bg-card)', borderRadius: 10 }}>
              <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-muted)', marginBottom: 10 }}>EDIT GROUP</div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
                ...
                <input className="input" value={editForm.name} style={{ flex: 1 }}
                  onChange={e => setEditForm(p => ({ ...p, name: e.target.value }))} placeholder="Group name" />
              </div>
              <input className="input" value={editForm.description} style={{ marginBottom: 10 }}
                onChange={e => setEditForm(p => ({ ...p, description: e.target.value }))} placeholder="Description (optional)" />
              <div style={{ display: 'flex', gap: 8 }}>
                <button className="btn btn-primary" onClick={saveGroupEdits} disabled={saving} style={{ flex: 1, padding: '8px' }}>
                  {saving ? 'Saving...' : '💾 Save'}
                </button>
          ...
          {details.members?.map(m => (
            <div key={m.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 0', borderBottom: '1px solid var(--border)' }}>
              ...
              {isAdmin && m.id !== currentUser.id && (
                <button className="btn-icon" title="Remove member" style={{ fontSize: 14, color: 'var(--danger)' }}
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: 16, display: 'flex', flexDirection: 'column', gap: 6 }}>
        ...
        {!loading && messages.length === 0 && (
          <div style={{ textAlign: 'center', color: 'var(--text-muted)', marginTop: 40, fontSize: 14 }}>No messages yet. Say something! 👋</div>
        )}
        {messages.map((msg, i) => {
            ...
            <div key={msg.id || i} style={{ display: 'flex', justifyContent: isMe ? 'flex-end' : 'flex-start', alignItems: 'flex-end', gap: 6 }}>
              ...
              <div style={{ maxWidth: '65%' }}>
                ...
                <div style={{ background: isMe ? 'var(--accent-primary)' : 'var(--bg-card)', padding: '10px 14px', borderRadius: 16, borderBottomRightRadius: isMe ? 4 : 16, borderBottomLeftRadius: isMe ? 16 : 4 }}>
                  ...
                  <div style={{ fontSize: 10, color: 'rgba(255,255,255,.5)', textAlign: 'right', marginTop: 3 }}>
      ...
      <form onSubmit={send} style={{ display: 'flex', gap: 10, padding: '12px 16px', borderTop: '1px solid var(--border)', background: 'var(--bg-secondary)', flexShrink: 0 }}>
        <input className="input" style={{ flex: 1, borderRadius: 24 }}
          placeholder="Message group…" value={input}
